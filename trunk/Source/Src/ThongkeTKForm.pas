...
begin
//[NXHop sua]
  ElRadioGroup1.Items[0]:=Utf8Decode('Nợ đầu kỳ');
  ElRadioGroup1.Items[1]:=Utf8Decode('Có đầu kỳ');
  ElRadioGroup1.Items[2]:=Utf8Decode('Nợ phát sinh');
  ElRadioGroup1.Items[3]:=Utf8Decode('Có phát sinh');
  ElRadioGroup1.Items[4]:=Utf8Decode('Nợ cuối kỳ');
  ElRadioGroup1.Items[5]:=Utf8Decode('Có cuốikỳ');
//
  SetCaption(Self);
  ...
  qryTKPhatsinh.ParamByName('branch_id').Value:= vpBranch_id;
  qryTKPhatsinh.Open;
end;
...
begin
  ...
  begin
    Suachungtu(qryDocListAnyWhere.FieldByName('DOC_ID').Value, qryDocListAnyWhere.FieldByName('PERIOD_ID').Value,qryDocListAnyWhere.FieldByName('BRANCH_ID').Value);
    qryDocListAnyWhere.Close;
    if IsPosted then
    begin

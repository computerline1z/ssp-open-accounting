...
type
  TCHITIETPBSPFrm = class(TForm)
    ...
    qryHESOSANPHAM: TWideStringField;
    grdKHACCOUNT_NAME: TdxDBGridMaskColumn;
    grdKHSANPHAM: TdxDBGridMaskColumn;
...
var
  CHITIETPBSPFrm: TCHITIETPBSPFrm;
...
procedure TCHITIETPBSPFrm.FormCreate(Sender: TObject);
begin
  locForm.BeginUpdate;
...
procedure TCHITIETPBSPFrm.BitBtn4Click(Sender: TObject);
begin
...
procedure TCHITIETPBSPFrm.grdKHKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  ProcessKeyDownInGrid(grdKH, Key, Shift, False,3);
end;

procedure TCHITIETPBSPFrm.grdKHMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TCHITIETPBSPFrm.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_F1 then
    if HtmlHelp(Self.handle,'HDSDKT.chm::/Thietlaptinhgiathanh.htm',HH_DISPLAY_TOPIC,0)=0 then
...
procedure TCHITIETPBSPFrm.dxDBPopupEdit1Enter(Sender: TObject);
begin
  MainDM.CurObjTypeID:=6;
end;

procedure TCHITIETPBSPFrm.FormShow(Sender: TObject);
begin
  SetVisible(Self);
end;

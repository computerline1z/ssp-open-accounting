...
type
  TDefBaocaoSamCondFrm = class(TForm)
...
var
  DefBaocaoSamCondFrm: TDefBaocaoSamCondFrm;
...
procedure TDefBaocaoSamCondFrm.FormCreate(Sender: TObject);
begin
...
procedure TDefBaocaoSamCondFrm.FormClose(Sender: TObject;
...
procedure TDefBaocaoSamCondFrm.dxDBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  ProcessKeyDownInGrid(dxDBGrid1, Key, Shift, True);
end;

procedure TDefBaocaoSamCondFrm.dxDBGrid1MouseUp(Sender: TObject;
...
procedure TDefBaocaoSamCondFrm.qryLCTTCondNewRecord(DataSet: TDataSet);
begin
  qryLCTTCondHESO.AsFloat:=1;
  qryLCTTCondID.AsInteger:= ID;
end;

procedure TDefBaocaoSamCondFrm.qryLCTTCondAfterInsert(DataSet: TDataSet);
begin
  qryLCTTCondTKNOBDV.FocusControl;
end;

procedure TDefBaocaoSamCondFrm.qryLCTTCondBeforePost(DataSet: TDataSet);
begin
...
procedure TDefBaocaoSamCondFrm.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_F1 then
...
procedure TDefBaocaoSamCondFrm.qryLCTTCondPostError(DataSet: TDataSet;
...
procedure TDefBaocaoSamCondFrm.TntBitBtn5Click(Sender: TObject);
begin
  if HtmlHelp(Self.handle,'HDSDKT.chm::/Thaotacthuongdung.htm',HH_DISPLAY_TOPIC,0)=0 then
    ShowMessageUnicode(182);
...
procedure TDefBaocaoSamCondFrm.qryLCTTCondDeleteError(DataSet: TDataSet;
  E: EDatabaseError; var Action: TDataAction);
begin
  Action := daAbort;
...
procedure TDefBaocaoSamCondFrm.qryLCTTCondBeforeDelete(DataSet: TDataSet);
begin
  if Not MainDm.CheckAllowDelete(DataSet,6) then Abort;
end;

procedure TDefBaocaoSamCondFrm.FormShow(Sender: TObject);
begin
  SetVisible(Self);

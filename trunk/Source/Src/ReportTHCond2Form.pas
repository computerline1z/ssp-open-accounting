...
var
  ReportTHCond2Frm: TReportTHCond2Frm;
...
procedure TReportTHCond2Frm.FormCreate(Sender: TObject);
begin
...
procedure TReportTHCond2Frm.FormClose(Sender: TObject;
...
procedure TReportTHCond2Frm.dxDBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  ProcessKeyDownInGrid(dxDBGrid1, Key, Shift, True);
end;

procedure TReportTHCond2Frm.dxDBGrid1MouseUp(Sender: TObject;
...
procedure TReportTHCond2Frm.qryLCTTCondNewRecord(DataSet: TDataSet);
begin
...
procedure TReportTHCond2Frm.qryLCTTCondAfterInsert(DataSet: TDataSet);
begin
  qryLCTTCondHESO.FocusControl;
end;

procedure TReportTHCond2Frm.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key=VK_F1 then
...
procedure TReportTHCond2Frm.qryLCTTCondPostError(DataSet: TDataSet;
...
procedure TReportTHCond2Frm.TntBitBtn5Click(Sender: TObject);
begin
  if HtmlHelp(Self.handle,'HDSDKT.chm::/Thaotacthuongdung.htm',HH_DISPLAY_TOPIC,0)=0 then
    ShowMessageUnicode(182);
...
procedure TReportTHCond2Frm.qryLCTTCondDeleteError(DataSet: TDataSet;
  E: EDatabaseError; var Action: TDataAction);
begin
  Action := daAbort;
...
procedure TReportTHCond2Frm.qryLCTTCondBeforeDelete(DataSet: TDataSet);
begin
  if Not MainDm.CheckAllowDelete(DataSet,6) then Abort;
end;

procedure TReportTHCond2Frm.FormShow(Sender: TObject);
begin
  SetVisible(Self);
...
procedure TReportTHCond2Frm.qryLCTTCondBeforePost(DataSet: TDataSet);
begin

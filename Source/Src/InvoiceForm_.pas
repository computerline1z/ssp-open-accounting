...
begin
 ...
 with MainDM do
  case qryInvoiceListLOAIHD.AsInteger of
    ...
    begin
      qryInvoiceList.Edit;
      qryInvoiceListLOAIHD.AsInteger:=1;
      rbMV.Checked:=True;
    end;
    btnPrint.Enabled:=qryInvoiceListLOAIHD.AsInteger=-1;
  end;
...
begin
  ...
  begin
    ...
    if ((Str='5') or (Str='7')) or ((strdebit = '11') and (str <> '3')) then
    if ((MainDM.qryInvoiceListOBJECT_ID.IsNull) or (qryCurDocEntry.FieldByName('DEBIT_OBJ').IsNull) or (qryCurDocEntry.FieldByName('DEBIT_OBJ').Value=MainDM.qryInvoiceListOBJECT_ID.Value)) then
    begin
      ...
      begin
        ...
        if (Not qryCurDocEntry.FieldByName('TENYTCO').IsNull) and
           ((qryCurDocEntry.FieldByName('CREDIT_TYPE').value <> 2) or (qryCurDocEntry.FieldByName('CREDIT_TYPE').value <> 12))then
        ...
        tinhgiatrithue;
        //////////qryItemsDETAIL_PRICE_VAT.Value :=
